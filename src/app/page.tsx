...
function MovieSearchPage() {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
        ...
        let url;
        if (currentSearch) {
            url = `https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&query=${encodeURIComponent(currentSearch)}&language=en-US&page=${currentPage}`;
        } else {
  ...
  const handlePageChange = (newPage: number) => {
    if (newPage < 1) return;
    const queryParam = currentSearch ? `query=${encodeURIComponent(currentSearch)}&` : '';
...
{isLoading ? (
  ...
  <div className="text-center col-span-full py-10">
    ...
    <p className="text-muted-foreground mt-2">
      We couldn&apos;t find any movies matching your search. Please try a different keyword.
    </p>
  </div>
)}
